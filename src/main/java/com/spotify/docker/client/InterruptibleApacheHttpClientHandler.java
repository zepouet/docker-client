 ...
 * threadpool.
 */
class InterruptibleApacheClient4Handler extends TerminatingClientHandler {

  private final URI baseUri;

  private final ExecutorService executor;

  InterruptibleApacheClient4Handler(final URI baseUri, final ExecutorService executor) {
  ...
  private class RequestTask implements Callable<ClientResponse> {
    ...
    public RequestTask(final ClientRequest cr) {
      ...
      SocketFactoryService sfs = SocketFactoryService.getInstance();
      SocketFactory sf = (SocketFactory) sfs.getSchemeSocketFactory(cr.getURI().getScheme());
      if (sf != null) {
